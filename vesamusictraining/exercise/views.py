...
def exercise(request):
  return render_to_response("exercise.html", context_instance=RequestContext(request))

@login_required
def list_lectures(request):
  complete_info = dict([(ue.lecture_name, ue) for ue in  UserLecture.objects.filter(user=request.user)])
  full_info = []
...
def lecture(request, lecture_name):
  if request.method == 'GET':
    ...
    # Ugly kludge
    ei = int(request.GET["exercise_index"]) if "exercise_index" in request.GET else 0
    ...
    if ei == first_exercise_idx:
      try:
        ue = UserLecture.objects.get(user=request.user, lecture_name = l.title)
...
def verify(request, lecture_name):
  if request.method == 'GET':
    l = Lecture.objects.get(title=lecture_name)
    ei = int(request.GET["exercise_index"]) + l.exercise_set.order_by("pk")[0].pk
    e = l.exercise_set.get(pk=ei)
    try:
      if e.question_type == "audio":
    ...
    log = Log(user=request.user, entry="Lecture %s, Exercise %d: %s" % (l.title, e.pk, repr(correct)))
    log.save()
