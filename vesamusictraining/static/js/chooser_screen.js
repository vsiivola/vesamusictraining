...
var chooser_screen_context,
    lang = "fi"; // FIXME, het the lang from Django
...
function ChooserScreen(mainWindow) {
    ...
    this.render_full = function () {
        $("#maintitle").html(gettext("Music Training | Choose your exercise"));
        ...
        for (i = 0; i < clist.length; i += 1) {
            ...
            if (lecture.instructions) {
                tstring += '<H3>' + gettext("Hints") + '</H3><p>' + lecture.instructions + '</p>';
            }

            if (lecture.outside_info_name) {
                tstring += '<H3>' + gettext("More information") +
                    '</H3>' + '<a class="outside" href="' +
                    lecture.outside_info_link +
                    '"> ' + lecture.outside_info_name + "</a>";
            }

            tstring += '<H3>' + gettext("Exercise information") + '</H3>';
            tstring += '<ul class="lecture_info">';
            tstring += '<li>' + gettext("version") + " " + lecture.version + '</li>';
            tstring += '<li> ' +  lecture.num_exercises + ' ' + gettext("exercises") + '</li>';
            if (lecture.complete) {
                tstring += '<li>' + gettext("completed") + '<ul>';
                tstring += '<li>' + gettext("version") + ' ' + lecture.version + '</li>';
                tstring += '<li>' + gettext("score") + ' ' + lecture.score + '</li>';
                tstring += '<li>' + gettext("on") + ' ' + lecture.complete_date + "</li></ul>";
            }
            tstring += '</ul>';
            tstring += '</div>';
            j += 1;
        ...
        for (i = 0; i < lclist.length; i += 1) {
            ...
            obj.value = gettext("Enter") + $('<div/>').html(" &raquo;").text();
